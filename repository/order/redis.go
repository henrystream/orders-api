...
import (
	...
	"github.com/henrystream/orders-api/model"
)
...
func (r *RedisRepo) Insert(ctx context.Context, order model.Order) error {
	...
	res := txn.SetNX(key, string(data), 0) //SetNX(ctx, key, string(data), 0)
...
func (r *RedisRepo) Update(ctx context.Context, order model.Order) error {
	...
	key := orderIDKey(order.OrderID)
	err = r.Client.SetXX(key, string(data), 0).Err() //SetNX(ctx, key, string(data), 0)
	if errors.Is(err, redis.Nil) {
		return ErrNotExist
	} else if err != nil {
...
func (r *RedisRepo) FindAll(ctx context.Context, page FindAllPage) (FindResult, error) {
	res := r.Client.SScan("orders", uint64(page.Offset), "*", int64(page.Size))
